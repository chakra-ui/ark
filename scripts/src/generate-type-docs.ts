...
async function extractPropertiesOfTypeName(
	...
	for (const typeStatement of typeStatements) {
		...
		for (const property of type.getProperties()) {
			...
			const defaultValue = property
				.getJsDocTags()
				.filter((tag) => tag.name === "default")[0]?.text?.[0].text;
	...
	const foo = Object.fromEntries(
		Object.entries(results)
			.filter(
				([key]) =>
					(!key.startsWith("Use") && key.endsWith("Props")) ||
					key.endsWith("Emits"),
			)
...
function shouldIgnoreProperty(property: ts.Symbol) {
	...
	const isExternal =
		...
		!sourceFileName?.includes("@zag-js");
...
const extractTypesForFramework = async (framework: string) => {
		...
		await Promise.all(
			components.map(async (component) => {
				const fileContent = await readFile(
					path.join(
						component,
						framework === "solid" ? "index.tsx" : "index.ts",
					),
					{
						encoding: "utf8",
					},
	...
	const result = await Promise.all(
		Object.entries(componentExportMap).flatMap(
			async ([component, typeExports]) => {
							...
							Object.fromEntries(
								Object.entries(x)
									...
									.map((x) => {
										...
										return [
											newName,
											{ props: Object.fromEntries(Object.entries(x[1])) },
										];
									})
									// @ts-expect-error
		...
		result.map(async (foo) => {
			...
			const elementMap = files
				.map((file) => {
					...
					const name = kebabToPascal(
						path.parse(file).name.replace(path.basename(component), ""),
					);
