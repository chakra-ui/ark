...
const main = async () => {
  const components = await globby(['../packages/react/src/components/*/*.{tsx,vue}'])
  ...
  components
    ...
    .map((item) => {
      ...
      const types = readJsonSync(`../website/src/content/types/react/${component}.types.json`)
      ...
      writeJsonSync(`../website/src/content/types/react/${component}.types.json`, content, {
        spaces: 2,
      })
    })
}
