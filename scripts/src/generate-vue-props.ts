...
const extractTypes = (component: string) => {
	...
	const propperties = sourceFile
		.getInterfaceOrThrow(
			`Use${chain(component).camelCase().capitalize().value()}Props`,
		)
		.getType()
		.getProperties()
	...
	outputFile.addImportDeclaration({
		...
		namespaceImport:
			camelCaseComponent === "switch" ? "zagSwitch" : camelCaseComponent,
		isTypeOnly: true,
	});
...
const main = async () => {
	...
	Promise.all(
		components
			...
			.filter(
				(x) =>
					!["format", "toast", "highlight", "frame", "client-only"].includes(x),
			)
			.map(extractTypes),
	);
};
