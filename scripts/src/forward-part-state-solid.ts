...
const project = new Project({
  tsConfigFilePath: join(root, 'packages/solid/tsconfig.json'),
  skipAddingFilesFromTsConfig: true,
})
const partFiles = globbySync('packages/solid/src/components/*/*.tsx', {
  cwd: root,
  absolute: true,
...
for (const path of partFiles) {
  ...
  if (/PolymorphicProps<[^>]*,\s*\w+>/.test(text)) {
...
for (const p of plans) {
  console.log(`  ${p.file.replace(`${root}/packages/solid/src/components/`, '')}`)
  console.log(
  ...
  for (const p of plans) {
    try {
      ...
      base.getExtends().forEach((e) => {
        const m = /^PolymorphicProps<('[a-z0-9]+')>$/.exec(e.getText())
        if (m) e.replaceWithText(`PolymorphicProps<${m[1]}, ${p.iface}>`)
      })
