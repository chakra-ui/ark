...
In Angular, import the standalone component from `@ark-ui/angular/presence` and pass the content through an
`ng-template`.
...
@Component({
  ...
  template: `
    <button type="button" (click)="present.set(!present())">Toggle</button>
