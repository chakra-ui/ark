...
For Angular, import the standalone root and indicator components from `@ark-ui/angular/swap`.

```ts
import { ArkSwapIndicatorComponent, ArkSwapRootComponent } from '@ark-ui/angular/swap'
```
...
Angular uses elements and property bindings for the same API:

```html
<ark-swap [swap]="swapped()" lazyMount unmountOnExit>
  <ark-swap-indicator type="on">...</ark-swap-indicator>
  <ark-swap-indicator type="off">...</ark-swap-indicator>
</ark-swap>
```
