...
To set up the segmented control correctly, you'll need to understand its anatomy and how we name its
parts.
...
Learn how to use the `SegmentGroup` component in your project. Let's take a look at the most basic
example:
...
prop and update it when the `onValueChange` event handler is called:
...
The `RootProvider` component provides a context for the radio-group. It accepts the value of the
`useRadio-group` hook. You can leverage it to access the component state and methods from outside
the radio-group.
