...
const AvatarNextImage = (props: ImageProps) => {
  ...
  // exclude hidden prop from ark image props
  const { hidden, ...arkImageProps } = avatar.getImageProps()

  // get next image props
  const { props: nextImageProps } = getNextImageProps(props)

  // merge props
  const mergedProps = { ...arkImageProps, ...nextImageProps }
