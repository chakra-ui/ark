...
`Listbox.List` as `ListVirtualizer.Root` and each `Listbox.Item` as `ListVirtualizer.Item`, and pass `scrollToIndexFn`
so keyboard navigation can scroll to options that are not mounted yet.
