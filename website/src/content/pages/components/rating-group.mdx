...
Allow `0.5` value steps by setting the `allowHalf` prop to `true`. Ensure to render the correct icon if the `half`
...
The `RootProvider` component provides a context for the rating group. It accepts the value of the `useRatingGroup`
