...
For large tree views with thousands of nodes, render only the visible rows with the
[Virtualizer](/docs/utilities/virtualizer).

Key implementation details:

- Use the `useTreeView` hook with `TreeView.RootProvider` for programmatic control
- Use `getVisibleNodes()` to get the flattened list of currently visible nodes, and keep the virtualizer's `count` in
  sync with it
- Render `TreeView.Tree` as `ListVirtualizer.Root` and each `TreeView.Node` as `ListVirtualizer.Item`
