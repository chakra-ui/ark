...
For very large lists, render only the visible options with the [Virtualizer](/docs/utilities/virtualizer). Render
`Combobox.List` as `ListVirtualizer.Root` and each `Combobox.Item` as `ListVirtualizer.Item`, keep the virtualizer's
`count` in sync with the filtered collection, and pass `scrollToIndexFn` so keyboard navigation can scroll to options
