...
Use the `FileUpload.ClearTrigger` component to provide users with a way to remove all uploaded files at once. This trigger will clear both accepted and rejected files from the upload component.
...
The example below demonstrates comprehensive error handling with file size limits (1KB-1MB), file count limits (max 3), type restrictions (images and PDFs), and how to display both accepted and rejected files with detailed error messages.
