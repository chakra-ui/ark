...
For very large collections, render only the visible options with the [Virtualizer](/docs/utilities/virtualizer). Render
`Select.List` as `ListVirtualizer.Root` and each `Select.Item` as `ListVirtualizer.Item`, and pass `scrollToIndexFn` so
keyboard navigation can scroll to options that are not mounted yet.
...
When virtualizing a large collection, avoid `Select.HiddenSelect`: it renders an `<option>` for every item in the
collection. Create a lightweight hidden input instead:
