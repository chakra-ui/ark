...
![CleanShot 2025-06-05 at 14.20.25@2x.png](attachment:f4f3cd98-f96c-4628-8a16-154d03ee85b1:CleanShot_2025-06-05_at_14.20.252x.png)
...
- Add **size variants** (e.g., `sm`, `md`) that adjust dimensions and font size

Essentially, recipes help you group related styles together and reuse them in a consistent way.

### Setting Up Global Styles

When building a design system, maintaining consistency is key. To support that, we’ll create a `shared` directory to
house global styles that can be reused across components.

Here’s what to include:

**CSS Reset Styles:** Start with a CSS reset to remove default browser inconsistencies. This ensures every component starts from the same baseline, no matter the browser. [Access the code here] 
...
**Design Tokens:** To keep your styles scalable, define reusable CSS variables for things like color, border radius,
etc. Add these to a `tokens.css` file.
...
**Keyframe Animations:** Define reusable keyframe animations like `slide-fade-in` and `slide-fade-out` . These will be
used to give the menu smooth transitions.
...
Inside the `shared` folder, create a `global.css` file and import all the file styles:
...
Finally, make sure to import `shared/global.css` at the root of your app to ensure all global styles are loaded
correctly.
...
Ark UI exposes helpful attributes like `data-scope="menu"` and `data-part="..."` for every part of the menu, making it
easy to target each piece directly in CSS.
...
Notice the use of the **`--menu-content-padding`** CSS variable. We'll define the actual value of this variable when we
set up our size variants shorty.
...
`--menu-item-font-size`, and `--menu-item-padding`. These variables act as placeholders, allowing us to control
item-specific properties such as spacing between elements, text size, and internal padding. The actual values for these
variables will be dynamically set by the size variants we’ll define later.

**Highlighted & Disabled States**
...
You can also combine these styles with checkbox/radio behavior:
...
To style each part of the menu component, we’ll create a **slot recipe** using Panda’s `sva()` function. This approach
lets us apply scoped styles to all the distinct parts of the menu, like `item`, `content`, `separator`, `indicator`, and
more.
...
To explore all of the menu styles and their variations, be sure to check out the Storybook examples.
