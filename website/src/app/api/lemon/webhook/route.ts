...
export const POST = async (request: NextRequest) => {
	const program = pipe(
		Effect.all([
			...
			Effect.fromNullable(
				Buffer.from(request.headers.get("x-signature") as string, "utf8"),
			),
		]),
		Effect.filterOrFail(
		...
		Effect.catchTags({
			ParseError: () =>
				Effect.succeed({ status: 400, message: "Payload is invalid" }),
			InternalServerError: () =>
				Effect.succeed({ status: 500, message: "Invalid signature" }),
		}),
	);
