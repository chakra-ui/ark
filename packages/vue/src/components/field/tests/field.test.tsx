...
describe("Field", () => {
	...
	it("should set textbox as disabled", async () => {
		...
		expect(document.querySelector('[data-part="root"]')).toHaveAttribute(
			"data-disabled",
		);
		expect(screen.getByText("Label")).toHaveAttribute("data-disabled");
		expect(screen.getByText("Some additional Info")).toHaveAttribute(
			"data-disabled",
		);
	});
	...
	it("should allow input to be controlled", async () => {
		render(ComponentUnderTest, {
			props: { modelValue: "Input is controlled" },
		});
