...
Pass the `@select` directive to the Menu component to perform some custom logic when
an item is selected. The callback is invoked with the `id` of the item.

```tsx
// <script>
const handleOnSelect: MenuProps['onSelect'] = (id) => console.log(id)

// <template>
<Menu @select="handleOnSelect">{/*...*/}</Menu>
```
