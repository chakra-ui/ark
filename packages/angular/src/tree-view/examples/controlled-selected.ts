...
@Component({
  selector: 'tree-view-controlled-selected-example',
  standalone: true,
  ...
  template: `
    <div arkTreeView [collection]="collection" [(selectedValue)]="selectedValue" #treeView="arkTreeView">
    ...
    <output>selected: {{ selectedValue()?.join(', ') || 'none' }}</output>
...
export class TreeViewControlledSelectedExample {
  readonly collection = fileTreeCollection
  readonly selectedValue = signal<string[] | undefined>(['package.json'])
