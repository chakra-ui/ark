...
@Directive({
  selector: '[arkTreeViewNodeCheckbox]',
  standalone: true,
  exportAs: 'arkTreeViewNodeCheckbox',
})
export class ArkTreeViewNodeCheckbox<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewNodeCheckbox', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getNodeCheckboxProps(nodeProps()),
    })
  }
}
