...
@Directive({
  selector: '[arkTreeViewItem]',
  standalone: true,
  exportAs: 'arkTreeViewItem',
})
export class ArkTreeViewItem<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewItem', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getItemProps(nodeProps()),
    })
  }
}
