...
@Directive({
  selector: '[arkTreeViewBranchIndentGuide]',
  standalone: true,
  exportAs: 'arkTreeViewBranchIndentGuide',
})
export class ArkTreeViewBranchIndentGuide<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewBranchIndentGuide', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getBranchIndentGuideProps(nodeProps()),
    })
  }
}
