...
@Directive({
  selector: '[arkTreeViewBranchTrigger]',
  standalone: true,
  exportAs: 'arkTreeViewBranchTrigger',
})
export class ArkTreeViewBranchTrigger<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewBranchTrigger', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getBranchTriggerProps(nodeProps()),
    })
  }
}
