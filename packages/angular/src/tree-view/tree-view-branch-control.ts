...
@Directive({
  selector: '[arkTreeViewBranchControl]',
  standalone: true,
  exportAs: 'arkTreeViewBranchControl',
})
export class ArkTreeViewBranchControl<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewBranchControl', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getBranchControlProps(nodeProps()),
    })
  }
}
