...
@Directive({
  selector: '[arkTreeViewBranchText]',
  standalone: true,
  exportAs: 'arkTreeViewBranchText',
})
export class ArkTreeViewBranchText<T extends TreeNode = TreeNode> {
  ...
  constructor() {
    ...
    const nodeProps = createTreeViewNodePropsResolver('ArkTreeViewBranchText', this.node, this.indexPath)
    applyArkProps({
      elementRef: inject(ElementRef),
      renderer: inject(Renderer2),
      destroyRef: inject(DestroyRef),
      props: () => context.api().getBranchTextProps(nodeProps()),
    })
  }
}
