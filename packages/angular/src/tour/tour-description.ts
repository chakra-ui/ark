...
@Directive({
  selector: '[arkTourDescription]',
  standalone: true,
  exportAs: 'arkTourDescription',
})
export class ArkTourDescription implements AfterContentInit {
  ...
  constructor() {
    ...
    applyArkProps({
      ...
      props: () => tour.api().getDescriptionProps(),
    })

    effect(() => {
      if (!this.ownsTextContent()) return
      this.renderer.setProperty(this.elementRef.nativeElement, 'textContent', tour.api().step?.description ?? '')
    })
  }
