...
@Directive({
  selector: '[arkTourProgressText]',
  standalone: true,
  exportAs: 'arkTourProgressText',
})
export class ArkTourProgressText implements AfterContentInit {
  ...
  constructor() {
    ...
    applyArkProps({
      ...
      props: () => tour.api().getProgressTextProps(),
    })

    effect(() => {
      if (!this.ownsTextContent()) return
      this.renderer.setProperty(this.elementRef.nativeElement, 'textContent', tour.api().getProgressText())
    })
  }
