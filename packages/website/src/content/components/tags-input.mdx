...
The TagsInput component consists of the `TagsInput.Item`, `TagsInput.ItemInput`,
`TagsInput.ItemText`, `TagsInput.Control`, `TagsInput.Input`,
`TagsInput.ClearTrigger`, `TagsInput.ItemDeleteTrigger` and `TagsInput.Label`
components. Combine them as desired to fit your design system.

Note that the `TagsInput.ClearTrigger` and `TagsInput.ItemDeleteTrigger`
components accept a single JSX element which will receive all necessary props to
add the event listeners and attributes to ensure the desired accessibility.
