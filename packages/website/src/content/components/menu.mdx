...
The Menu component consists of the `Menu.Arrow`, `Menu.ArrowTip`,
`Menu.Content`, `Menu.ContextTrigger`, `Menu.Item`, `Menu.ItemGroup`,
`Menu.ItemGroupLabel`, `Menu.OptionItem`, `Menu.Positioner`, `Menu.Separator`,
`Menu.Trigger` and `Menu.TriggerItem` components. Combine them as desired to fit
your design system.

Note that the `Menu.Trigger` and `Menu.ContextTrigger` components accept a
