...
- Resolved a problem where `Select.Indicator` was assigned to the wrong
  `data-part`.
...
- Resolved a problem where the `Dialog.CloseTrigger` was assigned to the wrong
  `data-part`.
- Fixed various issues for the `Toast` component that were caused by the API not
  being wrapped in an `Accessor`.
