...
- Added support to lazy mount the `DatePicker` and `ColorPicker` components
  using the `Presence` component

### Changed

- Improved accessibility of all disclosure components when lazy mounting is
  enabled.
...
- Add memoization to `Select` and `Combobox` item collection to improve
  performance.
