...
id: editable
name: Editable
description: Editable is an input field used for editing a single line of text. It renders as static text and transforms into a text input field when then edit interaction is triggered (click, focus, or double-click).
---
...
The Editable component consists of the `EditableArea`, `EditableCancelTrigger`, `EditableControl`,
`EditableEditTrigger`, `EditableInput`, `EditableLabel`, `EditablePreview` and
`EditableSubmitTrigger` components.
...
In some cases, you might need to use custom controls to toggle the edit and read mode. We use the
render prop pattern to provide access to the internal state of the component.
