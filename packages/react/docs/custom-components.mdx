...
In traditional UI frameworks, you usually get pre-designed, black-box components
components such as a `Pagination`. These components come with pre-configured
...
```tsx
import { type PaginationProps } from '@ark-ui/react'
